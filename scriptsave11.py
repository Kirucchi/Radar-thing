...
class Script:
    def __init__(self):
        ...
        self.DT_0 = 10
        self.pulse_data = 'railTestDiagonal.pkl'
        self.platform_position_data = 'UASSAR4_rail_diagonal.csv'
        self.given_object = 'triangle.csv'
        
        self.size = 50
        self.eyeballing_time_start = 272
        self.eyeballing_time_end = 1400

    def main(self, DT_0_param, pulse_data_param, platform_position_data_param, given_object_param, size_param, eyeballing_time_start_param, eyeballing_time_end_param):
        self.DT_0 = DT_0_param
        self.pulse_data = pulse_data_param
        self.platform_position_data = platform_position_data_param
        self.given_object = given_object_param
        self.size = size_param
        self.eyeballing_time_start = eyeballing_time_start_param
        self.eyeballing_time_end = eyeballing_time_end_param
        self.create_SAR_image(self.combine_all_arrays(),self.size)
    ...
    def unpack(self, file, legacy=False):
        """
        Unpacks PulsOn 440 radar data from input file
        """
        with open(file, 'rb') as f:
            config = self.read_config_data(f, legacy)
            ...
            start_range = SPEED_OF_LIGHT * ((scan_start_time * 1e-12) - self.DT_0 * 1e-9) / 2
    ...
    def extract_complex_pulse(self):
    ...
    def get_start_time_platform(self):
        pltpos = self.extract_platform_position()
        mini = 2 ** 63 -1
        maxi = -2 ** 63 -1
        for n in range(1000):
            if pltpos[n][2] > maxi:
    ...
    def get_end_time_platform(self):
        pltpos = self.extract_platform_position()
        mini = 2 ** 63 -1
        maxi = -2 ** 63 -1
        for n in range(1000):
    ...
    def combine_all_arrays(self):
        pickle_file = list()
        x = self.time_align_interpolation()
        pickle_file.append(x[1])
        pickle_file.append(x[0])
        pickle_file.append(self.extract_range_bins())
    ...
    '''
    def edge_detection(self, arr, diff):
    ...
    '''
    def part_image(self, start_x, start_y, end_x, end_y, pulses, range_bin_d, radar_x, radar_y, radar_z, size, resolution_multiplier):
        ...
        for ii in np.arange(0, yDiff):
            ...
            for jj in np.arange(0, xDiff):
                for kk in np.arange(0, 100):
                    distance = self.get_range(radar_x[kk], radar_y[kk], radar_z[kk], x, y)
    ...
    def linear_interp(self, pulses,range_bins,radar_x,radar_y,radar_z,size,center,width,height):
        ...
        for ii in range(size):
            ...
            for jj in range(size):
                for kk in range(len(radar_x)):
                    distance = self.get_range(radar_x[kk], radar_y[kk], radar_z[kk], x, y, 0) - first_range_bin
    ...
    def create_SAR_image(self, pickle_file,size):
        ...
        img = np.abs(self.linear_interp(pulses,range_bins,radar_x,radar_y,radar_z,size,static_object,5.0,4.0))
        ...
        while choice is not "4":
            ...
            if choice is "1":
                ...
                plt.imshow(self.edge_detection(img,(int)(ED_val)))
            elif choice is "3":
                self.gray_scale(img)
            if choice is not "4":
                plt.show()
    ...
    def intersect(self):
        cxpls = self.extract_complex_pulse()
        abscxpls = abs(cxpls)
        plt.imshow(abscxpls)
        #x = get_parabola()[0]
    ...
    def show_image_determine_start_time(self):
        self.intersect()
        
    def time_align_interpolation(self):
        cxpls = self.extract_complex_pulse()
        abscxpls = abs(cxpls)
        pltpos = self.extract_platform_position()
        strplat = self.get_start_time_platform()
        strrad = self.eyeballing_time_start
        stamprad = list(map(float,self.extract_time_stamp()))
        stamppla = self.extract_time_stamp2()
        endplat = self.get_end_time_platform()
        ...
        for elements in range(len(cutcxpls)):
            ...
            if floor + 1 < len(cutpltpos):
                ...
                if index == None:
                    index = n
            n += 1
        endrad = self.eyeballing_end_time - strrad
        #print(endrad)
        cxpls = cxpls[strrad:][:]
        if index != None:
            cxpls = cxpls[:index+1][:]
    ...
    def test(self):
        ...
        x = self.time_align_interpolation()[1]
        print(len(x))
    ...
    def get_parabola(self):
        ...
        distance = list()
        x = list()
        interm = self.get_start_time_highest_intensity()[1]
        adjust = self.get_start_time_highest_intensity()[0]
    ...
    def get_start_time_pulses(self): 
        cxpls = self.extract_complex_pulse()
        abscxpls = abs(cxpls)
        mini = 2 ** 63 -1
        maxi = -2 ** 63 -1
    ...
    def get_start_time_highest_intensity(self): 
        cxpls = self.extract_complex_pulse()
        abscxpls = abs(cxpls)
        maxi = -2 ** 63 -1
    ...
    def average_5_pixels(self, x,y,abscxpls): 
        return (abscxpls[x-2][y] + abscxpls[x-1][y] + abscxpls[x][y] + abscxpls[x+1][y] + abscxpls[x+2][y]) / 5.0
    
    def debug(self):
        temp = abs(self.extract_complex_pulse())
        strrad = self.eyeballing_time_start
        temp = abs(self.extract_complex_pulse())
        temp2 = temp[strrad:(strrad+500)][:]
        plt.imshow(temp2,extent=[0,10,0,500],aspect = 'auto')
        distancelist = list()
        x = self.extract_given_object()
        y = np.array(self.time_align_interpolation()[1])
        for ii in range(y.shape[0]):
            distance = self.get_range(x[0],x[1],x[2],y[ii][0],y[ii][1],y[ii][2])
            distancelist.append(distance)
        temp = distancelist[0:-1:round(y.shape[0] / temp.shape[0])]
        index = range(len(temp)+self.eyeballing_time_start,self.eyeballing_time_start,-1)
