...
def create_SAR_image(pickle_file):
    ...
    print(radar_y)
    size = 210
    pixel_values = np.zeros((size,size), dtype=complex)
    pixel_values2 = np.zeros((size,size))
    y = 3.5
    temp = y
    ...
    for ii in range(size):
        ...
        for jj in range(size):
            for kk in range(len(radar_x)):
                distance = get_range(radar_x[kk], radar_y[kk], radar_z[kk], x, y, 0)
                ratio = (distance % range_bin) / range_bin
                index = math.floor(range_bin)
    ...
    with open('pixel_values', 'rb') as f:
        pickle.dump(f,pixel_values2)
